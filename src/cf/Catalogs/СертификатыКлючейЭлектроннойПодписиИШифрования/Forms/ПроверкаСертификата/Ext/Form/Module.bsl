	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		Если Не РезультатПроверки Тогда
			...
			Если ПредупреждениеПослеПроверкиУЦ = Неопределено Тогда
				...
				Если НачалоЭлемента = "Расшифровка" И ЭлементНадписьОшибка.Заголовок = Форма.Элементы["Подписание" + ?(
					НаСервере, "ОшибкаСервер", "ОшибкаКлиент") + "Надпись"].Заголовок Тогда
					
					Форма.Элементы[ИмяНадписиРешение].Видимость = Форма.Элементы["Подписание" + ?(НаСервере, "РешениеСерверНадпись", "РешениеКлиентНадпись")].Видимость;
					Форма.Элементы[ИмяНадписиРешение].Заголовок = НСтр("ru = 'См. пункт Подписание данных.'");
					...
					Если ЭтоИзвестнаяОшибка Тогда
						...
						Если ЗначениеЗаполнено(ОписаниеИзвестнойОшибки.ДействияДляУстранения) Тогда
							...
							Если ПараметрыДополненияТекстаРешенияОшибкиКлассификатора.ПроверитьСертификатВЛокальномХранилищеКлиента Тогда
								Форма.ПроверитьСертификатВЛокальномХранилищеКлиента = НачалоЭлемента;
							КонецЕсли;
						КонецЕсли;
