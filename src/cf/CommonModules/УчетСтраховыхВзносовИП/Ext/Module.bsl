...
Функция РассчитатьФиксированныеСтраховыеВзносыПосле2023(Знач Период, Организация = Неопределено)
	...
	СтраховыеВзносы.ЕдиныйТариф = Цел(СуммаВзносаЕдиныйТариф * 100)/100; // Округление не может быть в большую сторону
	СтраховыеВзносы.ЕдиныйТарифВсего = Цел(СуммаВзносаЕдиныйТарифВсего * 100)/100;
...
Функция ВзносыСДоходовПодлежащиеУплате(Организация, НалоговыйПериод) Экспорт
	...
	СтруктураДоходов = СтруктураДоходовПоВидамДеятельности(Организация, НачалоГода(ПериодПрошлыйГод), КонецГода(ПериодПрошлыйГод));
	ВзносыНачислено = СтраховыеВзносыСДоходовКУплате(Организация, КонецГода(ПериодПрошлыйГод), СтруктураДоходов);
	Результат.ЗаПредыдущийГодИсчислено = ВзносыНачислено.СуммаВзносаПФРсДоходовВсего;
...
Функция ТаблицаТарифовСтраховыхВзносов(Организация, ДатаНачала, ДатаОкончания)
	...
	// Рассчитываем до 6 знаков - для тарифа военные пенсионеры коэффициент указывается с такой точностью
	ОписаниеТипаЧисло = ОбщегоНазначения.ОписаниеТипаЧисло(15, 6);
...
Функция СтраховыеВзносыУплаченные2024(ПараметрыРасчета)
	...
	СтраховойПериодДоЕНС = НачалоГода(НастройкиУчетаКлиентСервер.ДатаПереходаНаЕдиныйНалоговыйПлатеж() - 1);
	...
	УпрощенныйЗачет = УчетСтраховыхВзносовИППовтИсп.УпрощенныйЗачетФиксированныхВзносов(Организация, ПериодРасчета);
	...
	Для Каждого ТекущаяСтрока Из СведенияОбУплатеНалогов Цикл
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.ЗадолженностьДоЕНС)
			И ПриниматьКУчетуЗадолженностьДоЕНС(ТекущаяСтрока.ДатаПринятияЕНП) Тогда
			
			ЗаполнитьЗначенияСвойств(Результат.Добавить(), ТекущаяСтрока);
		КонецЕсли;
		
		Если УпрощенныйЗачет Тогда
			// Для УСН доходы и ПСН:
