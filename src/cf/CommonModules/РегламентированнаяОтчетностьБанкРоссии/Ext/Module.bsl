...
Функция СтрИндексОбщейСтроки(ДобавляемаяСтрока, ПараметрыВыгрузки)
	...
	Если УзелВсеСтроки.ЛокальноеИмя = "sst" Тогда
		...
		Если СтрИндОбщейСтроки = Неопределено Тогда
			...
			СтрКоличествоУникальныхСтрок = Формат(ИндОбщихСтрок.Количество(), "ЧН=; ЧГ=");
			ИндОбщихСтрок.Вставить(ДобавляемаяСтрока, СтрКоличествоУникальныхСтрок);
			
			СтрИндОбщейСтроки = СтрКоличествоУникальныхСтрок;
			
			НовКоличествоУникальныхСтрок = Число(" " + УзелВсеСтроки.ПолучитьАтрибут("uniqueCount")) + 1;
			УзелВсеСтроки.УстановитьАтрибут("uniqueCount", Формат(НовКоличествоУникальныхСтрок, "ЧН=; ЧГ="));
...
Функция ЗначениеПоФорматуВопроса(Знач Значение, ВопросАнкеты, ПараметрыВыгрузки) Экспорт
	...
	ИмяСправочника           = ВопросАнкеты["QUESTION_DICTNAMEID"];
	ИмяСправочникаНСИ        = ВопросАнкеты["QUESTION_NSI"];
	ПредставлениеСправочника = ВопросАнкеты["QUESTION_NSIVIEWTYPE"];
