...
Функция ПриЗагрузкеОтчетаОРозничныхПродажах(Устройство, ДанныеОПродажах, Отказ, СообщениеОбОшибке) Экспорт
	...
	Для Каждого ОтчетОПродажах Из ДанныеОПродажах Цикл
		...
		Для Каждого ЧекиПродажи Из ОтчетОПродажах.Чеки Цикл
			...
			Для Каждого СтрокаТЧ Из ЧекиПродажи.Товары Цикл
				Если СтрокаТЧ.ПризнакСпособаРасчета = Перечисления.ПризнакиСпособаРасчета.Аванс  Тогда
					Если ЗначениеЗаполнено(ВидОплатыПодарочныйСертификатСобственный) 
						И ЧекиПродажи.ТипРасчета =  Перечисления.ТипыРасчетаДенежнымиСредствами.ПриходДенежныхСредств Тогда 
					...
					ИначеЕсли НЕ ОтражатьАвансыКакСертификаты Тогда
						ИмяРеквизитаСумма = ?(
							ЧекиПродажи.ТипРасчета =  Перечисления.ТипыРасчетаДенежнымиСредствами.ПриходДенежныхСредств, 
							"Сумма", "СуммаВозврат");
			...
			Для каждого СтрокаОплаты Из ЧекиПродажи.Оплаты Цикл
				...
				ИначеЕсли СтрокаОплаты.СуммаПредоплатой > 0 И НЕ ОтражатьАвансыКакСертификаты 
					И ЧекиПродажи.ТипРасчета = Перечисления.ТипыРасчетаДенежнымиСредствами.ПриходДенежныхСредств Тогда // поддерживаем зачет аванса только при продаже
		...
		ПродажаСертификатов.Свернуть("ВидОплаты", "Сумма");
		...
		ТаблицыДанных.Вставить("ПодарочныеСертификаты",     ПродажаСертификатов);
		ТаблицыДанных.Вставить("Предоплаты",                ТаблицаПредоплаты);
		ТаблицыДанных.Вставить("ЗачетПредоплаты",           ТаблицаЗачетПредоплаты);
