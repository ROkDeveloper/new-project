...
Функция МетодыВзаимодействия() Экспорт
	...
	ДобавитьОписаниеМетода(ВсеМетоды,
		ИменаМетодов.СоздатьТокенАвторизацииЛКАУСН,
		НСтр("ru = 'Создание токена авторизации'"),
		"POST",
		"/auth/ticket/");
...
Функция ВыполнитьОбменССервисомЛКАУСН() Экспорт
	...
	Ошибки = НовыйОписаниеОшибокОтправки();
	ОтправитьРасчетыНДФЛАУСНВБанк(Ошибки);
	
	// Методы, которые нужно выполнить при обмене с ЛК АУСН
	ИменаМетодов = ИнтеграцияАУСНКлиентСервер.ИменаМетодовВзаимодействияИнтеграцияСЛК();
	МассивМетодов = Новый Массив;
	МассивМетодов.Добавить(ИменаМетодов.ОтправитьРасчетНДФЛ);
...
Функция ВыполнитьМетодВзаимодействияСЛК(ПараметрыМетода) Экспорт
	...
	ИначеЕсли ПараметрыМетода.МетодВзаимодействия = МетодыВзаимодействия.ОтправитьРасчетНДФЛ Тогда
		Возврат ОтправитьРасчетыНДФЛВФНС(ПараметрыМетода.Организация);   
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПолучитьНеуплаченныеНалоговыеНачисления(Организация)
	...
	Метод = ИнтеграцияАУСНКлиентСервер.ИменаМетодовВзаимодействияИнтеграцияСЛК().ПолучитьНеуплаченныеНалоговыеНачисления;
	ПараметрыМетода = ПараметрыМетодаВзаимодействияИнтеграцияСЛК(
		Метод, Новый Структура("Организация", Организация));
	Ответ = ОтветСервисаЛК(ПараметрыМетода);
...
// Запрос к сервису АУСН для отправки расчетоа НДФЛ в ФНС
//
// Параметры:
//  Организация - СправочникСсылка.Организации
// 
...
//
Функция ОтправитьРасчетыНДФЛВФНС(Организация)
	...
	ЕстьОшибки = Ложь;
	Пока ДокументКОтправке.Следующий() Цикл
		Если Организация <> ДокументКОтправке.Организация Тогда
			Продолжить;
		КонецЕсли;
		ПараметрыМетода = НовыеПараметры(
			ИнтеграцияАУСНКлиентСервер.ИменаМетодовВзаимодействияИнтеграцияСЛК().ОтправитьРасчетНДФЛ);
...
//
Функция НовыйТокенАвторизацииИнтеграцииСЛКАУСН(ИдентификаторПриложения, СообщениеОбОшибке = "") Экспорт
	...
	Тикет = ТикетАутентификацииНаПорталеПоддержкиДляАУСН(СообщениеОбОшибке);
	Если Не ЗначениеЗаполнено(Тикет) Тогда
		Возврат Неопределено;
	КонецЕсли;

	ПараметрыМетода = ИнтеграцияАУСНКлиентСервер.НовыеПараметрыМетодаВзаимодействия(
		ИнтеграцияАУСНКлиентСервер.ИменаМетодовВзаимодействияИнтеграцияСЛК().СоздатьТокенАвторизацииЛКАУСН);
	...
	Если ТокенАвторизации <> Неопределено Тогда
		...
		ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(
			ВладелецДанныхБезопасногоХранилищаИнтеграцииСЛК(ИдентификаторПриложения), ТокенАвторизации,
			КлючДанныхБезопасногоХранилищаИнтеграцииСАУСН());
		УстановитьПривилегированныйРежим(Ложь);
	КонецЕсли;
...
Процедура ЗаписатьСостояниеИнтеграцииЛКАУСН(ОтветСервиса, Организация)
	...
	Если ОбъектXDTO = Неопределено Тогда
		ИмяСобытия = ИнтеграцияАУСНКлиентСервер.ИмяСобытияЖурналаРегистрации();
		ТекстОшибки = НСтр("ru = 'Не удалось расшифровать ответ сервиса ФНС по привязке НП.
						   |Ответ не содержит XDTO-объекта PostAskBindPartnerWithInnResponse.'");
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, , , ТекстОшибки);
		Возврат;
	КонецЕсли;
...
Функция ОтветСервисаПоНомеруСообщения(НомерСообщения, ТокенАвторизации, ТокенПортала, ИдентификаторПриложения)
	...
	ПараметрыЗапроса = ПараметрыЗапросаКСервису(ПараметрыМетода, ОписаниеМетода, ТокенАвторизации, ТокенПортала);
	ОтветСервиса = ОтветСервиса(ПараметрыЗапроса);
...
Функция ОтветСервисаЛК(ПараметрыМетода)
	...
	СообщениеОбОшибке = "";
	ТокенАвторизации = ТокенАвторизацииАУСН(ИдентификаторПриложения, СообщениеОбОшибке);
	...
	Если Ответ.Статус = СтатусыЗапросов.Ошибка И Ответ.КодСостояния = 401 Тогда
		СообщениеОбОшибке = "";
		ТокенАвторизации = НовыйТокенАвторизацииИнтеграцииСЛКАУСН(ИдентификаторПриложения, СообщениеОбОшибке);
	...
	Ответ = ОтветСервисаПоНомеруСообщения(НомерСообщения, ТокенАвторизации, ТокенПортала, ИдентификаторПриложения);
...
Процедура ЗаполнитьПараметрыЗапросаКСервисуЛКАУСН(ПараметрыЗапроса, ПараметрыМетода, ОписаниеМетода, ТокенАвторизации, ТокенПортала)
	
	ВсеМетоды = ИнтеграцияАУСНКлиентСервер.ИменаМетодовВзаимодействияИнтеграцияСЛК();
	Если ОписаниеМетода.Метод = ВсеМетоды.СоздатьТокенАвторизацииЛКАУСН Тогда
		// Параметры не требуются
		Возврат;
	КонецЕсли;
	
	Если ТокенАвторизации = Неопределено Тогда
		ПараметрыЗапроса.Вставить("ТекстЗапроса", ПараметрыМетода.ТекстЗапроса);
	Иначе
		...
		Иначе
			Заголовки.Вставить("UserToken", ТокенАвторизации);
		КонецЕсли;
...
Функция ПараметрыЗапросаКСервису(ПараметрыМетода, ОписаниеМетода, ТокенАвторизации = Неопределено,
	...
	Если ЭтоМетодИнтеграцииСЛК(ОписаниеМетода.Метод) Тогда 
		ЗаполнитьПараметрыЗапросаКСервисуЛКАУСН(
			ПараметрыЗапроса, ПараметрыМетода, ОписаниеМетода, ТокенАвторизации, ТокенПортала);
	Иначе
...
Функция ИмяСервисаЛКАУСННаПорталеИТС()

	Возврат "LkUsnIntegration";
...
Процедура ЗаполнитьДанныеЗапроса(Данные, ПараметрыМетода)
	...
	ИначеЕсли Метод = МетодыИнтеграцииСЛК.СоздатьТокенАвторизацииЛКАУСН Тогда
		ЗаполнитьДанныеЗапроса_СоздатьТокенАвторизации(Данные, ПараметрыМетода);
	КонецЕсли;
...
Функция ТикетАутентификацииНаПорталеПоддержкиДляАУСН(СообщениеОбОшибке)

	УстановитьПривилегированныйРежим(Истина);
	ДанныеАутентификации = ИнтернетПоддержкаПользователей.ТикетАутентификацииНаПорталеПоддержки(
		ИмяСервисаЛКАУСННаПорталеИТС());
	УстановитьПривилегированныйРежим(Ложь);
...
Функция ТокенАвторизацииАУСН(ИдентификаторПриложения, СообщениеОбОшибке = "") Экспорт

	УстановитьПривилегированныйРежим(Истина);
	ТокенАвторизации = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(
		ВладелецДанныхБезопасногоХранилищаИнтеграцииСЛК(ИдентификаторПриложения),
		КлючДанныхБезопасногоХранилищаИнтеграцииСАУСН());
	УстановитьПривилегированныйРежим(Ложь);
	...
	Если ЗначениеЗаполнено(ТокенАвторизации) Тогда
		Возврат ТокенАвторизации;
	Иначе
		Возврат НовыйТокенАвторизацииИнтеграцииСЛКАУСН(ИдентификаторПриложения, СообщениеОбОшибке);
	КонецЕсли;
...
Функция КлючДанныхБезопасногоХранилищаИнтеграцииСАУСН()

	Возврат "serviceUserTokenAUSN";
