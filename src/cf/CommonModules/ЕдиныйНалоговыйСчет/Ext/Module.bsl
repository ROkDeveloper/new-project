...
Функция ПолучитьРасчетыПоНалогамПоСпискуЗадач(ПараметрыДокумента) Экспорт
	...
	ПоследнийСрокУведомления = КонецГода(Период);
	...
	МассивДатУплаты.Добавить(ПоследнийСрокУведомления);
	...
	Если Период >= НачалоМесяца(Период) И Период <= ОтдельныйСрокУведомления Тогда // сроки подготовки отчетности по НДФЛ, удержанному с 23 числа по конец месяца (кроме декабря)
	...
	ИначеЕсли Период > ОбщийСрокУведомления И Период >= НачалоМесяца(КонецГода(Период)) И Период <= КонецГода(Период) Тогда // сроки подготовки отчетности по НДФЛ, удержанному с 23 по 31 декабря
			...
			Для каждого НоваяСтрока Из ТаблицаНачисленияНалоговНаЕНС Цикл
				...
				ИначеЕсли ТипЗнч(Выборка.Регистратор) = Тип("ДокументСсылка.РегламентнаяОперация") Тогда
					Если НоваяСтрока.СчетУчета = ПланыСчетов.Хозрасчетный.НДС
						...
						Или НоваяСтрока.СчетУчета = ПланыСчетов.Хозрасчетный.НДФЛ_ИП_НалоговаяБазаСвышеПредельнойВеличины 
...
Функция НалогНаПрибыльЕжемесячноПоРасчетнойПрибыли(Организация, Период)
	...
	|	ХозрасчетныйОбороты.Субконто2 КАК Субконто2,
	|	ХозрасчетныйОбороты.Субконто3 КАК Субконто3,
	|	ВЫБОР КОГДА -ХозрасчетныйОбороты.СуммаОборот > 0 ТОГДА -ХозрасчетныйОбороты.СуммаОборот ИНАЧЕ 0 КОНЕЦ КАК СуммаНалога,
...
Функция НалогНаПрибыльКварталПоРасчетнойПрибыли(Организация, Период)
	...
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, , Счет = &СчетНалога, , Организация = &Организация) КАК ХозрасчетныйОбороты
	|	ГДЕ
	|		ХозрасчетныйОбороты.Субконто1 = ЗНАЧЕНИЕ(Перечисление.ВидыПлатежейВГосБюджет.Налог)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, , , Счет = &СчетНалога, , Организация = &Организация) КАК ХозрасчетныйОстаткиИОбороты
	|	ГДЕ
		...
		Для каждого СтрокаПоВидуДоговора Из СтрокаПоКонтрагенту.Строки Цикл
			...
			Для ИндексСтроки = 0 По КоличествоОплат - 1 Цикл
				...
				Для каждого СтрокаНачисления Из СтрокаПоВидуДоговора.Строки Цикл
					...
					СтрокаОплаты.ВидАгентскогоДоговора = СтрокаНачисления.ВидДоговора;
					СтрокаОплаты.Сумма = Мин(ОстатокСуммы, СтрокаНачисления.Сумма);
					СтрокаОплаты.Начислено = СтрокаОплаты.Сумма;
