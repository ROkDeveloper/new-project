...
Процедура ЗаполнитьРеглОтчет(ИмяФормы,
	...
	Иначе
		...
		Если ПараметрыАвтоформирования.Свойство("ПоОбособленномуПодразделению") И ПараметрыАвтоформирования.ПоОбособленномуПодразделению
			...
			ОбработатьЗагруженныеДанные(ДанныеРеглОтчета.СтруктураРеквизитовФормы, РазделыОтчета.Титульный);
			ИзменитьОКТМОРазделов(ДанныеРеглОтчета.СтруктураРеквизитовФормы, РазделыОтчета.Титульный);
			СформироватьРаздел1_2(ДанныеРеглОтчета, РазделыОтчета.Титульный);
			СформироватьРаздел1_1(ДанныеРеглОтчета, РазделыОтчета.Титульный);
...
Процедура ОбработатьЗагруженныеДанные (СтруктураРеквизитовФормы, СтруктураДанныхТитульный) Экспорт
	...
	ОчиститьПриложения4КЛисту02НеВключенныеВСоставОтчета(СтруктураРеквизитовФормы);
	
КонецПроцедуры

Процедура ОчиститьПриложения4КЛисту02НеВключенныеВСоставОтчета(СтруктураРеквизитовФормы) Экспорт
...
Процедура ИзменитьОКТМОРазделов(СтруктураРеквизитовФормы, СтруктураДанныхТитульный, ТабличныйДокумент = Неопределено, ИмяРаздела = Неопределено) Экспорт
...
Процедура СформироватьРаздел1_1(ДанныеРеглОтчета, СтруктураДанныхТитульный, ТабличныйДокумент = Неопределено) Экспорт
			...
			Для каждого ОКТМО Из ПереченьОКТМО Цикл
				...
				Иначе
					...
					Если ЕстьЛист02_5_ЗаполненныйПоОП Тогда
						...
						Если ЕстьЛист02_5_РС_1 Тогда
							...
							Для каждого Лист02 Из Листы02 Цикл
								...
								+ РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(ДанныеРеглОтчета, Листы02_5, "П000250010003", Усл)
								- РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(ДанныеРеглОтчета, Листы02_5, "П000250011003", Усл);
							КонецЦикла;
					...
					Иначе
						...
						= РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(ДанныеРеглОтчета, Листы02, "П000020027103", Усл)
						- РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(ДанныеРеглОтчета, Листы02, "П000020028103", Усл);
			...
			Для каждого ОКТМО Из ПереченьОКТМО Цикл
				...
				Иначе
					...
					Для каждого Лист02 Из Листы02 Цикл
						Листы02_6 = Лист02.Строки[5].Строки;
						СуммаНалогаВБюджСубРФ = СуммаНалогаВБюджСубРФ
						+ РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(ДанныеРеглОтчета, Листы02_6, "П000260010003", Усл)
						- РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(ДанныеРеглОтчета, Листы02_6, "П000260011003", Усл);
					КонецЦикла;
...
Процедура СформироватьРаздел1_2(ДанныеРеглОтчета, СтруктураДанныхТитульный, ТабличныйДокумент = Неопределено) Экспорт
		...
		Если ЗаполнятьРаздел1_2 Тогда
			...
			Для каждого ОКТМО Из ПереченьОКТМО Цикл
				...
				ИначеЕсли ОтчетныйМесяц = 9 Тогда
					Если АвансовыеПлатежиРазличаются(ДанныеРеглОтчета, СтруктураДанныхТитульный, ТабличныйДокумент, ОКТМО) Тогда
				...
				Иначе //ОтчетныйМесяц = 11
					Если ЕстьАвансовыеПлатежи(ДанныеРеглОтчета, СтруктураДанныхТитульный, ТабличныйДокумент, ОКТМО) Тогда
						СочетанияЗначений.Добавить(Новый Структура("КодКвартала, ОКТМО", "21", ОКТМО));
					Иначе
						СочетанияЗначений.Добавить(Новый Структура("КодКвартала, ОКТМО", "", ОКТМО));
					КонецЕсли;
...
Функция АвансовыеПлатежиРазличаются(ДанныеРеглОтчета, СтруктураДанныхТитульный, ТабличныйДокумент, Знач ОКТМО = Неопределено)
...
Функция ЕстьАвансовыеПлатежи(ДанныеРеглОтчета, СтруктураДанныхТитульный, ТабличныйДокумент, Знач ОКТМО = Неопределено)
