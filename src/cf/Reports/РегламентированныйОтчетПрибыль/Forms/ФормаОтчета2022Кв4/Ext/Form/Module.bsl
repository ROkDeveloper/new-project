...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено
		...
		Если СтруктураРеквизитовФормы.мСкопированаФорма <> Неопределено Тогда
			...
			ОбъектОтчета.СформироватьРаздел1_2(ЭтаФорма, СтруктураДанныхТитульный, ТабличныйДокумент);
			
			ОбъектОтчета.ИнициализироватьМногоуровневыйРаздел(СтруктураРеквизитовФормы.мВыбраннаяФорма,
			СтруктураРеквизитовФормы, мСтруктураМногоуровневыхРазделов, "Раздел1_1");
			
			ОбъектОтчета.СформироватьРаздел1_1(ЭтаФорма, СтруктураДанныхТитульный, ТабличныйДокумент);
...
Процедура ВосстановитьСохраненныеДанные()
	...
	ОбъектОтчета(ЭтаФорма.ИмяФормы).ОчиститьПриложения4КЛисту02НеВключенныеВСоставОтчета(СтруктураРеквизитовФормы);
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
		...
		Для каждого Лист02 Из Листы02 Цикл
			...
			Если ЗначениеЗаполнено(Лист02.Данные["ПризнакВключения"]) Тогда
				...
				Если ЭтоКонсолидированнаяГруппа Тогда
					...
					Для каждого Лист02_Поиск Из Листы02 Цикл
						Листы02_6_Поиск = Лист02_Поиск.Строки[5].Строки;
						Стр269 = Стр269 + РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(Форма, Листы02_6_Поиск, "П000260009803");
					КонецЦикла;
				...
				ИначеЕсли ЕстьОбособленныеПодразделения Тогда
					...
					Для каждого Лист02_Поиск Из Листы02 Цикл
						Листы02_5_Поиск = Лист02_Поиск.Строки[4].Строки;
						Стр269 = Стр269 + РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(Форма, Листы02_5_Поиск, "П000250009803", Усл);
					КонецЦикла;
				...
				Иначе
					...
					Для каждого Лист02_Поиск Из Листы02 Цикл
						...
						Стр269 = Стр269 + РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(Форма, Листы02_7А_Поиск, "П0027А0005003",
			...
			Для каждого Лист02_5 Из Листы02_5 Цикл
				...
				Если ЗначениеЗаполнено(Лист02_5.Данные["ПризнакВключения"]) Тогда
					...
					Для каждого Лист02_Поиск Из ДеревоДанных.Строки Цикл
						...
						Лист02_7А_Стр050 = Лист02_7А_Стр050 + РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(Форма, Листы02_7А, "П0027А0005003",
	...
	ИначеЕсли ИмяРаздела = "Лист02_7Б" Тогда
		...
		Листы02_Стр120 = РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(Форма, ДеревоДанных.Строки, "П000020012003");
		Листы02_Стр200 = РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(Форма, ДеревоДанных.Строки, "П000020020003");
...
Процедура СформироватьРаздел1_1()
	...
	ОбъектОтчета(ЭтаФорма.ИмяФормы).СформироватьРаздел1_1(ЭтаФорма, СтруктураДанныхТитульный, ТабличныйДокумент);
...
Процедура СформироватьРаздел1_2()
	...
	ОбъектОтчета(ЭтаФорма.ИмяФормы).СформироватьРаздел1_2(ЭтаФорма, СтруктураДанныхТитульный, ТабличныйДокумент);
...
Процедура ОбработатьНаличиеОтсутствиеОбособленныхПодразделений(ИмяРаздела)
	...
	Иначе
		Для каждого Лист02 Из ДеревоДанных.Строки Цикл
			Листы02_5 = Лист02.Строки[4].Строки;
			Если РегламентированнаяОтчетностьКлиентСервер.ВеткаУдовлетворяющаяУсловиямПоиска(Листы02_5, Новый Структура("ПризнакВключения", "V")) <> Неопределено Тогда
				ЕстьОбособленныеПодразделения = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
