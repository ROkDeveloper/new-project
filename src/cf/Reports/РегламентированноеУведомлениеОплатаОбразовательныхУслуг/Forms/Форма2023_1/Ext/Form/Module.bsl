...
Функция ЗаполнитьАвтоНаСервере()
	...
	ПараметрыОтчета.Вставить("ОтчГод", ДанныеУведомления.Титульная.ОтчГод); 
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	Для Каждого КЗ Из СтруктураДанных.Контейнер.СведОплОбрУсл Цикл 
		...
		ДанныеМногостраничныхРазделов.СведОплОбрУсл.Добавить(СтруктураСтраницы);
	КонецЦикла;
	
	Если СтруктураДанных.Контейнер.СведОплОбрУсл.Количество() = 0 Тогда
		ДанныеМногостраничныхРазделов.СведОплОбрУсл.Добавить(ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтруктураДанных.Контейнер.СведОплОбрУсл.Добавить()));
		ДанныеМногостраничныхРазделов.СведОплОбрУсл[0].Значение.УИД = Новый УникальныйИдентификатор;
	КонецЕсли;
	...
	СтрРег.Многостраничность = Истина;
	СтрРег.Многострочность = Истина;
	Для Каждого Стр Из ДанныеМногостраничныхРазделов.СведОплОбрУсл Цикл 
		СтрРегТр = СтрРег.ПолучитьЭлементы().Добавить();
		СтрРегТр.Наименование = "Стр. " + (ДанныеМногостраничныхРазделов.СведОплОбрУсл.Индекс(Стр) + 1);
		...
		УведомлениеОСпецрежимахНалогообложенияКлиентСервер.ИмяСтраницыВДеревеЖурналаСправок(
			ДеревоСтраниц, Стр.Значение.УИД, ДанныеМногостраничныхРазделов["СведОплОбрУсл"]);
	КонецЦикла;
