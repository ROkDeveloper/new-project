...
Процедура ПриОткрытии(Отказ)
	...
	#Если ВебКлиент Тогда
		Этаформа.ПодключитьОбработчикОжидания("ВыполнитьПроверкуПодключения", 0.1, Истина);
	#Иначе
...
Процедура ВыполнитьПроверкуПодключения()
	...
	Если НЕ ПроверкаВыполнена = Истина Тогда
		Если ДокументооборотСКОКлиентСервер.ПодсистемаУдалитьЦБСуществует() Тогда
			Оповещение = Новый ОписаниеОповещения("ПриОткрытииПослеВыполненияПроверки", ЭтотОбъект);
		...
		Иначе
			ПриОткрытииПослеВыполненияПроверки(Новый Структура("Выполнено,ВыполнениеРазрешено", Истина, Ложь), Неопределено);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ПриОткрытииПослеВыполненияПроверки(Результат, ВходящийКонтекст) Экспорт
