...
Процедура ПодготовитьФормуНаСервере()
	...
	Если Параметры.Свойство("Содержание") Тогда
		...
		Содержание.УстановитьФорматированнуюСтроку(Параметры.Содержание.ПолучитьФорматированнуюСтроку());
	...
	Если Параметры.Свойство("Вложения") И ЗначениеЗаполнено(Параметры.Вложения) Тогда
		...
		Для Каждого Вложение Из Параметры.Вложения Цикл
			ИмяИРасширениеФайла = Новый Структура;
			ИмяИРасширениеФайла.Вставить("Имя", Вложение.Представление);
			Если Вложение.ВидДанных = "Текс" Тогда
				ИмяИРасширениеФайла.Вставить("Расширение", ".txt");
				
				ПотокВПамяти = Новый ПотокВПамяти();
				Текст = Новый ЗаписьТекста(ПотокВПамяти, КодировкаТекста.UTF8, , Символы.ПС);
				Текст.Записать(Вложение.Данные);
				Текст.Закрыть();
				...
				ПоместитьФайлыБезРасширенияНаСервере(АдресХранилища, ИмяИРасширениеФайла);
			ИначеЕсли Вложение.ВидДанных = "ДвоичныеДанные" Тогда
				ИмяИРасширениеФайла.Вставить("Расширение", ".xml");
				АдресХранилища = ПоместитьВоВременноеХранилище(Вложение.Данные, Новый УникальныйИдентификатор);
				ПоместитьФайлыБезРасширенияНаСервере(АдресХранилища, ИмяИРасширениеФайла);
			КонецЕсли;
		КонецЦикла;
