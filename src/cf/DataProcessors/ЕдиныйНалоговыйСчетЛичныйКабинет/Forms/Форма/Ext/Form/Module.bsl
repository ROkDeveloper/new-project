...
&НаКлиенте
Процедура ДекорацияСообщитьОПроблемеНажатие(Элемент)
	Если ИспользоватьБТС Тогда
		СообщитьОбПроблемеЧерезБТС();
	ИначеЕсли ИспользоватьБИП И ПроверитьЗаполнение() Тогда
		СообщитьОбПроблемеЧерезБИП();
	КонецЕсли;
КонецПроцедуры
...
Процедура ПереключитьСтраницыПередОбработкойДанных()
	...
	Элементы.БаннерСервисПерегружен.Видимость              = Ложь;
...
Процедура СообщитьОбОшибкеЧерезБИП()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СообщенияВСлужбуТехническойПоддержки") Тогда
		...
		Вложения = ВложениеЖурналЗагрузкиЕНС(Объект.Организация);
		ЕстьВложениеЖурналаЗагрузки = ЗначениеЗаполнено(Вложения);
		...
		ДанныеСообщения.Сообщение            = СообщениеПользователяОбОшибке(Истина, ЕстьВложениеЖурналаЗагрузки, Истина);
...
Процедура СообщитьОбОшибкеЧерезБТС()
	...
	ДополнитьВложениеЖурналомРегистрации(Вложения);
	ЕстьЗаписиЖурналаРегистраций = Вложения.Количество() > КоличествоВложений;
	
	Содержание = Новый ФорматированныйДокумент;
	Содержание.УстановитьФорматированнуюСтроку(Новый ФорматированнаяСтрока(СообщениеПользователяОбОшибке(ЕстьЗаписиЖурналаРегистраций, ЕстьВложениеЖурналаЗагрузки, Ложь)));
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("СоздаватьОбращение", Истина);
	ПараметрыФормы.Вставить("Содержание",         Содержание);
	ПараметрыФормы.Вставить("Вложения",           Вложения);
	ПараметрыФормы.Вставить("Тема",               ТемаСообщенияПользователяОбОшибке());
...
&НаКлиенте
Процедура СообщитьОбПроблемеЧерезБИП()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СообщенияВСлужбуТехническойПоддержки") Тогда
		...
		ДанныеСообщения = МодульСообщенияВСлужбуТехническойПоддержкиКлиентСервер.ДанныеСообщения();
		ДанныеСообщения.Получатель           = "v8";
		ДанныеСообщения.Тема                 = ТемаСообщенияПользователяДанныеОтсутствуют();
		ДанныеСообщения.Сообщение            = СообщениеПользователяОПроблеме(ЕстьВложениеЖурналаЗагрузки, Истина);
		ДанныеСообщения.ИНН                  = СведенияОбОрганизации.ИНН;
...
&НаКлиенте
Процедура СообщитьОбПроблемеЧерезБТС()
	
	Вложения = ВложениеЖурналЗагрузкиЕНС(Объект.Организация);
	ЕстьВложениеЖурналаЗагрузки = ЗначениеЗаполнено(Вложения);
	
	Содержание = Новый ФорматированныйДокумент;
	Содержание.УстановитьФорматированнуюСтроку(Новый ФорматированнаяСтрока(СообщениеПользователяОПроблеме(ЕстьВложениеЖурналаЗагрузки, Ложь)));
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("СоздаватьОбращение", Истина);
	ПараметрыФормы.Вставить("Содержание",         Содержание);
	ПараметрыФормы.Вставить("Вложения",           Вложения);
	ПараметрыФормы.Вставить("Тема",               ТемаСообщенияПользователяДанныеОтсутствуют());
...
Процедура СообщитьОНекорректныхДанныхЧерезБИП()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СообщенияВСлужбуТехническойПоддержки") Тогда
		...
		ДанныеСообщения.Сообщение            = СообщениеПользователяОбОшибкеВДанных(СведенияОбОрганизации, ЕстьВложениеЖурналаЗагрузки, Истина);
		ДанныеСообщения.ИНН                  = СведенияОбОрганизации.ИНН;
...
Процедура СообщитьОНекорректныхДанныхЧерезБТС()
	...
	Содержание = Новый ФорматированныйДокумент;
	Содержание.УстановитьФорматированнуюСтроку(Новый ФорматированнаяСтрока(СообщениеПользователяОбОшибкеВДанных(СведенияОбОрганизации,
		ЕстьВложениеЖурналаЗагрузки, Ложь)));
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("СоздаватьОбращение", Истина);
	ПараметрыФормы.Вставить("Содержание",         Содержание);
	ПараметрыФормы.Вставить("Вложения",           Вложения);
	ПараметрыФормы.Вставить("Тема",               ТемаСообщенияПользователяОНекорректныхДанных());
...
&НаКлиенте
Функция ТемаСообщенияПользователяДанныеОтсутствуют()
	
	Возврат НСтр("ru = 'Данные о состоянии единого налогового счета отсутствуют'");
...
&НаКлиенте
Функция СообщениеПользователяОбОшибке(ЕстьЗаписиЖурналаРегистраций, ЕстьВложениеЖурналаЗагрузки, ЕстьТехническаяИнформация)
	
	Сообщение = Новый Массив;
	Сообщение.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для автоматического разбора ошибок мы сформировали текст и приложили необходимую техническую информацию в приложенных файлах.
		|
		|================ Не удаляйте эту часть письма и приложенные файлы ================
		|
		|#ОшибкаЗагрузки
		|Произошла ошибка при получении данных из личного кабинета единого налогового счета
		|по организации ""%1"" ИНН: %2 КПП: %3 ОГРН: %4 Код НО: %5'"),
		Объект.Организация,
		СведенияОбОрганизации.ИНН,
		СведенияОбОрганизации.КПП,
		СведенияОбОрганизации.ОГРН,
		СведенияОбОрганизации.КодНО));
		Если ЕстьЗаписиЖурналаРегистраций Или ЕстьВложениеЖурналаЗагрузки Или ЕстьТехническаяИнформация Тогда
			Сообщение.Добавить(Символы.ПС);
			Сообщение.Добавить(НСтр("ru = 'К сообщению приложены файлы с подробной информацией об ошибке:'"));
		КонецЕсли;
		Если ЕстьВложениеЖурналаЗагрузки Тогда
			Сообщение.Добавить(Символы.ПС);
			Сообщение.Добавить(НСтр("ru = ' - записи из журнала загрузки единого налогового счет'"));
		КонецЕсли;
...
&НаКлиенте
Функция СообщениеПользователяОПроблеме(ЕстьВложениеЖурналаЗагрузки, ЕстьТехническаяИнформация)
	...
	Сообщение.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для автоматического разбора ошибок мы сформировали текст и приложили необходимую техническую информацию в приложенных файлах.
		...
		|
		|#ОтсутствуютДанные
		|Данные о состоянии единого налогового счета отсутствуют
		|по организации ""%1"" ИНН: %2 КПП: %3 ОГРН: %4 Код НО: %5'"),
		Объект.Организация,
		СведенияОбОрганизации.ИНН,
		СведенияОбОрганизации.КПП,
...
&НаСервереБезКонтекста
Функция СообщениеПользователяОбОшибкеВДанных(СведенияОбОрганизации, ЕстьВложениеЖурналаЗагрузки, ЕстьТехническаяИнформация)
	
	ДатаСообщения = ОбщегоНазначения.ТекущаяДатаПользователя();
	
	Сообщение = Новый Массив;
	Сообщение.Добавить(СтрШаблон(НСтр("ru = 'Организация: %1
			|ИНН: %2
			...
			|================ Не удаляйте эту часть письма и приложенные файлы ================
			|
			|Для автоматического разбора ошибок мы сформировали текст и приложили необходимую техническую информацию в приложенных файлах.
			|#НекорректныеДанные
			|Некорректные данные о состоянии единого налогового счета
			|по организации ""%1"" ИНН: %2 КПП: %5 ОГРН: %6 Код НО: %7'"),
...
&НаСервереБезКонтекста
Процедура ДополнитьВложениеЖурналомРегистрации(Вложения)
...
&НаСервереБезКонтекста
Функция ВложениеЖурналЗагрузкиЕНС(Организация)
...
Процедура ОформитьВнутренниеОшибки(КодОшибки)
	...
	// 107 - ошибка Интернет
	// 109 - недействительный ключ авторизации
	// 110 - недействительный токен авторизации
		...
		Если ЗначениеЗаполнено(НастройкиДоступностиЛичногоКабинетаЕНС) Тогда
			...
			Если Не ЗначениеЗаполнено(ПричинаБлокировки) Тогда
				...
				Элементы.ТекстПревышеноКоличествоЗапросов.Заголовок =
					СтрШаблон(ШаблонОписанияПревышеноКоличествоЗапросов, СведенияОбОрганизации.ИНН);
