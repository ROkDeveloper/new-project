...
//   Периодичность   - ПеречислениеСсылка.Переодичность - Переодичность уплаты взносов
...
Функция СтатусУплатыВзносов(ПараметрыЗадачи)
	...
	ПростойУчетЕНС = КонецГода(ПараметрыЗадачи.ПериодСобытия) = КонецКвартала(ПараметрыЗадачи.ПериодСобытия)
...
Функция СтатусУплатыВзносов2024(ПараметрыЗадачи)
	...
	СрокУплаты  = УчетСтраховыхВзносовИП.СрокОплатыВзноса(Организация, ПериодСобытия, Истина);
