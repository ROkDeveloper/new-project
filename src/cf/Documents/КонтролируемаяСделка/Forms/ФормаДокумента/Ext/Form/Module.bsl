...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокКодов = КонтролируемыеСделки.ПолучитьСписокКодовНаименованийСделки();
...
Функция ПолучитьПараметрыПредметаСделки(ПредметСделки, ВерсияУведомления)
	...
	Если ТипЗнч(ПредметСделки)=Тип("СправочникСсылка.Номенклатура") Тогда
		Предмет = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПредметСделки, "НаименованиеПолное, СтранаПроисхождения, Услуга, КодТНВЭД, КодОКП, КодОКВЭД, КодОКВЭД2, КодОКПД2");
		...
		ЗаполняемыеКодыПредметовСделок = ЗаполняемыеКодыПредметовСделок(Не Предмет.Услуга, ВерсияУведомления);
		ЗаполнитьЗначенияСвойств(ПараметрыПредмета, Предмет, ЗаполняемыеКодыПредметовСделок);
		
		Если Предмет.Услуга Тогда
		...
		КонецЕсли;
	Иначе
		Предмет = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПредметСделки, "Наименование, КодОКВЭД, КодОКВЭД2");
