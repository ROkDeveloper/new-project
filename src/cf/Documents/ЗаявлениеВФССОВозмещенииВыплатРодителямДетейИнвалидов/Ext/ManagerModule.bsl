...
Функция ТабличныйДокументДополнительныеДниОтпуска(МассивСсылок, ОбъектыПечати)
	...
	|	ЕСТЬNULL(ТЧОплаты.КоличествоДней, 0) КАК КоличествоДнейЗаМесяц,
	|	ЕСТЬNULL(ТЧОплаты.КоличествоЧасовЗаМесяц, 0) КАК КоличествоЧасовЗаМесяц,
	|	ЕСТЬNULL(ТЧОплаты.СреднедневнойЗаработок, 0) КАК ЗаработокЗаМесяц,
	...
	|	ЕСТЬNULL(ТЧОплаты.СреднедневнойЗаработокЗаГод, 0) КАК ЗаработокЗаГод,
...
Процедура ВывестиЗаявлениеОВозмещении_2024(Выборка, ТабличныйДокумент, ОбъектыПечати)
	...
	Пока Выборка.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		Для Каждого ДанныеШапкиПоСотруднику Из ТаблицаСотрудников Цикл
			...
			ВывестиТаблицуПриказов_2024(КонтекстПечати, ТаблицаСтрокПоСотруднику);
			ВывестиТаблицуЗаМесяц_2024(КонтекстПечати, ТаблицаСтрокПоСотруднику);
			ВывестиТаблицуНакопленных_2024(КонтекстПечати, ТаблицаСтрокПоСотруднику);
			ВывестиТаблицуЗаГод_2024(КонтекстПечати, ТаблицаСтрокПоСотруднику);
		КонецЦикла;
...
Процедура ВывестиТаблицуПриказов_2024(КонтекстПечати, ТаблицаСтрокПоСотруднику)
	// Таблица приказов.
	РазрешитьДобавлениеСтраницы(КонтекстПечати.ТабличныйДокумент);
	...
	ВложеннаяТаблица.Сортировать(ВложенныеИменаПолей);
	Если ВложеннаяТаблица.Количество() = 0 Тогда
		ВложеннаяТаблица.Добавить();
		ВложеннаяТаблица.Добавить();
		ВложеннаяТаблица.Добавить();
	КонецЕсли;
	Для Каждого СтрокаПриказа Из ВложеннаяТаблица Цикл
...
Процедура ВывестиТаблицуЗаМесяц_2024(КонтекстПечати, ТаблицаСтрокПоСотруднику)
	...
	ВложенныеИзмерения = "Месяц, ЕстьДниЗаМесяц";
	ВложенныеРесурсы   = "КоличествоДнейЗаМесяц, КоличествоЧасовЗаМесяц, ЗаработокЗаМесяц";
	...
	Для Каждого ВложеннаяСтрока Из ВложеннаяТаблица Цикл
		...
		Если ВложеннаяСтрока.ЕстьДниЗаМесяц Тогда
			...
			КоличествоДней = ВложеннаяСтрока.КоличествоДнейЗаМесяц;
			Заработок      = ВложеннаяСтрока.ЗаработокЗаМесяц;
	...
	КоличествоДнейИтог = ВложеннаяТаблица.Итог("КоличествоДнейЗаМесяц");
	...
	КонецЕсли;
	КоличествоЧасовИтог = ВложеннаяТаблица.Итог("КоличествоЧасовЗаМесяц");
...
Процедура ВывестиТаблицуНакопленных_2024(КонтекстПечати, ТаблицаСтрокПоСотруднику)
...
Процедура ВывестиТаблицуЗаГод_2024(КонтекстПечати, ТаблицаСтрокПоСотруднику)
	...
	ВложенныеИзмерения = "Месяц, ЕстьДниЗаГод";
	ВложенныеРесурсы   = "КоличествоДнейЗаГод, КоличествоЧасовЗаГод, ЗаработокЗаГод";
	ВложеннаяТаблица = ТаблицаСтрокПоСотруднику.Скопировать(Новый Структура("ЕстьДниЗаГод", Истина));
	ВложеннаяТаблица.Свернуть(ВложенныеИзмерения, ВложенныеРесурсы);
	ВложеннаяТаблица.Сортировать(ВложенныеИзмерения);
	Если ВложеннаяТаблица.Количество() = 0 Тогда
		ВложеннаяТаблица.Добавить();
	...
	Для Каждого ВложеннаяСтрока Из ВложеннаяТаблица Цикл
		...
		Если ВложеннаяСтрока.ЕстьДниЗаГод Тогда
			...
			Заработок      = ВложеннаяСтрока.ЗаработокЗаГод;
	...
	КоличествоДнейИтог = ВложеннаяТаблица.Итог("КоличествоДнейЗаГод");
	Если КоличествоДнейИтог > 0 Тогда
		УправлениеПечатьюБЗК.ВывестиЧислоПоБуквам(Секция, КоличествоДнейИтог, "КоличествоДнейЗаГодИтог");
	КонецЕсли;
	КоличествоЧасовИтог = ВложеннаяТаблица.Итог("КоличествоЧасовЗаГод");
	Если КоличествоЧасовИтог > 0 Тогда
