...
Функция ВерсияУведомленияПоОтчетномуГоду(ОтчетныйГод) Экспорт
	...
	Если ОтчетныйГод >= Дата(2021, 1, 1) Тогда
...
Процедура СформироватьКонтролируемыеСделки(Уведомление, СообщатьОПрогрессе)
	...
	СписокКодовСторонСделки = КонтролируемыеСделки.ПолучитьСоответствиеКодовСтороныСделки();
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДокументСделки = Неопределено ИЛИ
			...
			СписокКодовСтороныСделки = СписокКодовСторонСделки.Получить(Выборка.КодНаименованияСделки);
...
Процедура ЗаполнитьАдресаСделки(СведенияОСделке, Грузоотправитель, Грузополучатель, ВерсияУведомления)
...
Функция ПолучитьПараметрыАдресаСделки(Объект, ДатаСовершенияСделки)
	...
	ДанныеАдреса = Новый Структура("Страна, КодРегиона, Город, НаселенныйПункт", СтранаМира, "", "", "");
...
Функция ПолучитьОсновныеСведенияУведомленияДляВыгрузки(Уведомление) Экспорт
	...
	ЭлектроннаяПочтаОрганизации = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(Организация, Справочники.ВидыКонтактнойИнформации.EmailОрганизации);
