...
Функция НовыйФайлВыгрузки_503_504(Контекст, ИдентификаторФайла)
	...
	Если ОсновныеСведенияВыгрузки.ЭтоПБОЮЛ Тогда
		ЗаполнитьСведенияОФизлице_503_504(Контекст, ТекстФайла);
	Иначе
		ЗаполнитьСведенияОрганизации(Контекст, ТекстФайла);
	КонецЕсли;
...
Процедура ЗаполнитьСведенияОФизлице_503_504(Контекст, ТекстФайла)
...
Функция ТекстПоСделке502(Контекст, ЛистыУведомления, Лист1А)
	...
	ЗаполнитьСведенияОДоходахРасходах502_503_504(Контекст, ТекстСделки, Лист1А);
...
Функция ТекстПоСделке503_504(Контекст, ЛистыУведомления, Лист1А)
	...
	ЗаполнитьСведенияОснованияКонтроляСделки503_504(Контекст, ТекстСделки, Лист1А);
	...
	ЗаполнитьСведенияОбУчастникахФизЛицах503_504(Контекст, ТекстСделки, ЛистыРаздела3, ДанныеРаздела3, Лист1А.Сделка);
...
Процедура ЗаполнитьСведенияОснованияКонтроляСделки503_504(Контекст, ТекстСделки, Сведения)
...
Процедура ЗаполнитьСведенияОДоходахРасходах502_503_504(Контекст, ТекстСделки, Сведения)
...
Процедура ЗаполнитьСведенияОбУчастникахФизЛицах503_504(Контекст, ТекстСделки, Листы3, ДанныеРаздела3, Сделка)
...
Процедура ЗаполнитьСведенияОбУчастникахФизЛицах(ПрототипУзлаВыгрузки, Листы3, ДанныеРаздела3, Сделка)
	...
	Для Каждого Лист3 Из СписокЛистов3 Цикл
		...
		Если ПустаяСтрока(ФизЛицо.Строка030ИНН) Тогда
			...
			УстановитьЗначениеЭлемента(Узел_УдЛичнФЛ, "КодВидДок", КонтролируемыеСделки.ПолучитьКодВидаДокументаПоВидуДокумента(ФизЛицо.ВидДокумента));
			...
			ПоляАдреса = Новый Структура("Индекс, КодРегиона, Район, Город, НаселенныйПункт, Улица, Дом, Корпус, Квартира",
										"Индекс", "КодРегион", "Район", "Город", "НаселПункт", "Улица", "Дом", "Корпус", "Кварт");
